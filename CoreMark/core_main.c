...
#error "Please define a way to initialize a memory block."
    ...
    if (results[0].iterations == 0)
    {
        ...
        while (secs_passed < (secs_ret)1)
        {
            ...
            iterate(&results[0]);
            stop_time();
            secs_passed = time_in_secss(get_time());
        }
    ...
    ee_printf("Total time (secs): %f\n", total_time / 1000.0);
    if ((total_time / 1000.0) > 0)
        ee_printf("Iterations/Sec   : %f\n",
                  default_num_contexts * results[0].iterations
                      / (total_time / 1000.0));
#else
    ee_printf("Total time (secs): %f\n", time_in_secss(total_time));
    if (time_in_secss(total_time) > 0)
        ee_printf("Iterations/Sec   : %f\n",
                  default_num_contexts * results[0].iterations
                      / time_in_secss(total_time));
#endif
    if (time_in_secss(total_time) < 10)
    {
        ee_printf(
